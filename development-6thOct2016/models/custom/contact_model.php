...
class contact_model extends Model
{
   ...
   /**
    ...
    function get($contactID, $formatter = null)
    {
        ...
        $si = sql_prepare("select c.login, c.password, c.email, c.first_name, c.last_name, c.alt_first_name, c.alt_last_name,
    ...
    function update($contact, $prevData, $changingPassword = false)
    {
        ...
        if(!$error)
        {
            ...
            if($ret)
            {
                ...
                if(!$changingPassword)
                {
                    $newProfile = update_verified_contact($contact->login->value, $contact->password->value, $this->session->getSessionData('sessionID'));
                }
                else
                {
                    $newProfile = custlogin($contact->login->value, $contact->password->value, $this->session->getSessionData('sessionID'), $this->session->getSessionData('cookiesEnabled'));
                }
                if($newProfile && $newProfile->login !== null && $newProfile->login !== '' && $newProfile->login !== false)
                    update_cookie_expire($newProfile->login, time() + $this->session->getProfileCookieLength());
                ...
                $postHookData = array('returnValue'=>$newProfile);
    ...
    function getChannelFields($uniqueIdentifier = null)
    {
        ...
        {
            $baseQuery = "select ct.chan_type_id, l.label, c.username, c.userid from channel_types ct
                              left outer join labels l on l.tbl = %d and l.lang_id = %d and l.label_id = ct.chan_type_id
                              left outer join contact2channel_type c on c.c_id = ";
        ...
        {
            list($channelTypeID, $channelName, $userName, $userID) = $row;
            $channelFields[$channelTypeID] = new ChannelField($channelTypeID, $channelName, $userName, $userID);
        }
     ...
     * @param $user String The username of the contact
     * @param $pass String The password of the contact
     ...
     */
    function doLogin($user, $pass, $sessionID, $widgetID, $url)
    {
        ...
        $profile = $this->getProfileSid($user, $pass, $sessionID);
    ...
    function doLogout($currentUrl, $redirectUrl = null)
    {
        ...
        $logoutResult = custlogout_session($sessionID, $CI->session->getProfileData('cookie'));
     ...
     * @param $user String The username of the contact
     * @param $pass String The password of the contact: plaintext, non-encrypted
     * @param $sessionID String The session Id of the contact
     */
    function getProfileSid($user, $pass, $sessionID)
    {
        $CI = get_instance();
        $preHookData = array();
        ...
        AbuseDetection::check();
        $profile = $this->session->createMapping(custlogin($user, $pass, $sessionID, 1));
        $postHookData = array('returnValue'=>$profile);
        RightNowHooks::callHook('post_login', $postHookData);
        return $profile;
    }
	...
	function lookupContactByLogin($login, $cr_id)
	{
		...
		try {
                    ...
                    if ($cr_id === null || $cr_id === false || $cr_id === '') {
                        $roql .= " and C.c\$cr_affiliation IS NULL";
                    } else {
                        $roql .= " and C.c\$cr_affiliation = " . $cr_id;
                    }
		    // $roql_result = RNCPHP\ROQL::query( "SELECT C.Login,C.Emails.Address from Contact C WHERE C.Login ='".$login."' and C.c\$cr_affiliation = ".$cr_id ." ")->next();
                    $roql_result = RNCPHP\ROQL::query($roql)->next();
                }
                catch (RNC\ConnectAPIError $err) { echo $err->getMessage();}
    ...
    */
    ...
    static function answerNotification($answerId, $status, $contactId = null)
    {
        ...
        $si = sql_get_int(sprintf('SELECT count(*)
            FROM ans_notif
            WHERE (c_id = %d) AND (a_id = %d)',
            $contactId, $answerId));
        ...
        if (!$renewingAnswerNotif && intval($status) !== -4)
        {
        ...
        else
        {
            ...
            {
                $si = sql_prepare(sprintf('SELECT start_time
                    FROM ans_notif
                    WHERE (c_id = %d) AND (a_id = %d)',
                    $contactID, $answerID));
    ...
    function emailContactLogin($email)
    {
       ...
       $email = strtolower(within_sql($email));
        ...
        $si = sql_prepare("SELECT c_id FROM contacts
        ...
        $pairdata = array('c_id' => intval($row[c_id]), 'email' => $email);
    ...
    */
    function contactAlreadyExists($idType, $idValue, $accountSetup = false)
    {
        ...
        elseif($idType === 'login')
        {
            if($profile = $this->session->getProfile() && $profile->login->value === $idValue)
                return $result;
    ...
    */
    ...
    private function lookupContact($email, $firstName, $lastName)
    {
        ...
        $contact = checkCache("existingContactEmail$email");
        if($contact !== null)
            return $contact;
        ...
        setCache("existingContactEmail$email", $contact, true);
        return $contact;
    }
   ...
   /**
    ...
    /**
    * Checks if the contacts account is disabled
    * @assert only called when contact is resetting, i.e. not logged in
    * @param $contact Object The contact record to check
    * @return mixed True if they match or an error message otherwise
    */
    private static function checkValidAccount($contact)
    {
        $disabled = sql_get_int(
            sprintf("select disabled from contacts where (c_id = %d)", within_sql($contact->c_id->value)));

        if ($disabled)
            return getMessage(SORRY_THERES_ACCT_PLS_CONT_SUPPORT_MSG);
        return true;
    }
